...
class _SborkaScreenState extends State<SborkaScreen> {
  ...
  Future<void> _confirmDelete(BuildContext context, String docId, bool isIPK) async {
    ...
    final orderDoc = await FirebaseFirestore.instance.collection('Sborka').doc(docId).get();
    if (orderDoc.exists) {
    ...
    if (yes == true) {
      await FirebaseFirestore.instance.collection('Sborka').doc(docId).delete();
  ...
  Future<void> _switchSpec(int newSpec, String route) async {
    ...
    final user = FirebaseAuth.instance.currentUser;
    if (user != null) {
      String specText = newSpec == 2 ? 'Монтажник' : 'Пакетирование';
